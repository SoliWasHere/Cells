...
public class EntityTooltip {
    ...
    private static void addCellInfo(java.util.List<String> lines, Cell cell) {
        ...
        lines.add(String.format("Specialization: %.2f", cell.getSpecializationIndex()));
        
        lines.add("--- Chemistry (8D) ---");
        ChemicalSignature chem = cell.getChemicalPreference();
        ...
        lines.add("--- Stomach ---");
        double wasteMag = cell.getStomachWaste().magnitude();
        lines.add(String.format("Waste: %.1f / %.1f", wasteMag, cell.getWasteThreshold()));
    }
